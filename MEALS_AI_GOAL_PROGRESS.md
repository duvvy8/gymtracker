...
- **Gemini model currently selected:** `gemini-3.8-flash` by default, now user-switchable from the Meals photo panel between five Flash-class models (3.8, 3.7, 3.6, 3.5, 3.5 Lite). All accept image input, structured output and `thinkingLevel: MEDIUM`, so the request shape is identical for each. The choice persists in the Dexie settings row; the Worker validates it against a closed allow-list and falls back to the default.
...
- [x] No live Gemini calls in any check; the provider is stubbed and CI needs no key. **Corrected:** the browser QA posted a deliberately malformed image to the real endpoint, which the Worker forwarded to Gemini, so every QA run spent a request from the daily allowance and showed up as 400 BadRequest in the usage dashboard. It now sends an unsupported content type, failing at request validation before the provider is contacted. Verified: a full QA run makes no provider call.
...
- **2026-09-13 18:56 BST (Claude, QA quota leak):** The AI Studio usage dashboard showed 400 BadRequest errors alongside the 429s. Traced to the browser QA itself: its endpoint check posted a 9-byte fake PNG that passed signature validation and was forwarded to Gemini, so each QA run consumed one of the 20 daily requests. This broke the goal's rule that checks must never consume the user's quota. The check now uses an unsupported content type and asserts 415, proving the same calm-failure contract without contacting the provider. 16 of 16 QA checks still pass and the Worker log confirms no provider call.
- **2026-09-13 19:15 BST (Claude, model picker):** Added a model selector below the analyse controls, so hitting one model's daily cap no longer stops the feature.
  - The free tier meters `GenerateRequestsPerDayPerProjectPerModel`, so each model carries its own allowance. Five models therefore multiply the practical daily total rather than merely trading quality.
  - The candidate list came from the live `ListModels` response for this key, not from memory, and was filtered to stable Flash-class models that support `generateContent`, image input and `thinkingLevel`. Previews, aliases and Pro, audio, robotics and computer-use models were excluded.
  - Security: the browser sends `x-meal-model` as a request, never an instruction. The Worker narrows it through `resolveMealModel` before it is interpolated into the provider URL, so an unknown or hostile value silently becomes the default. Tested with `../../evil`, an absolute URL, an unlisted model and an empty string.
  - Persistence uses the existing settings row through `saveSettings`, not a new storage mechanism; the field is optional so rows and backups written before the picker still parse.
  - `check:meals` extended to 36 checks. Browser QA 16 of 16, with persistence confirmed across a reload.
